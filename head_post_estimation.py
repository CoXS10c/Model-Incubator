...
PARSER.add_argument(
    ...
    dest='img',
    help=
    'the image which would be used to calculate similarity between other images'
)
...
my_head_pose_estimator.load_yaw_variables(
    os.path.realpath("deepgaze/etc/tensorflow/head_pose/yaw/cnn_cccdd_30k.tf"))
...
for file_name in tqdm.tqdm(Path(ARGS.folder).glob('**/*')):
    ...
    yaw = my_head_pose_estimator.return_yaw(
        image)  # Evaluate the yaw angle using a CNN
    angle_dict[str(file_name)] = [
        float(roll[0, 0, 0]),
        float(pitch[0, 0, 0]),
        float(yaw[0, 0, 0])
    ]
    if ARGS.img:
        most_similar[str(file_name)] = cosineDS(angle_dict[str(file_name)],
                                                image_angel)
