...
def find_similar_img(img, topk=20):
    ...
    for extracted_file_path in tqdm.tqdm(extracted_img_size_folder.glob('*_0.jpg')):
        ...
        if suffix != '.png' and suffix != '.jpg' and suffix != '.jpeg':
            continue
        image = cv2.imread(str(extracted_file_path))  # Read the image with OpenCV
        # Get the angles for roll, pitch and yaw
        roll = my_head_pose_estimator.return_roll(
            image)  # Evaluate the roll angle using a CNN
        pitch = my_head_pose_estimator.return_pitch(
        ...
        yaw = my_head_pose_estimator.return_yaw(
            image)  # Evaluate the yaw angle using a CNN
        angle_dict[extracted_file_path] = [float(roll[0, 0, 0]),
...
def faceswap(src, dst, out):
    ...
    if not ARGS.warp_2d:
        ...
        if ARGS.correct_color:
            ...
            src_face = correct_colours(
                warped_dst_img, src_face, src_points)
        # Warp
...
if __name__ == "__main__":
    ...
    # _extract and _output folder is those img we would like to swapped
